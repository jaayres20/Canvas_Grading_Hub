/***************************************************************
 * CANVAS GRADING HUB – SINGLE FILE MASTER
 * Version 2.2  (Standalone, no Bootstrap, no GitHub)
 *
 * Features:
...
const CANVAS_HUB_VERSION = '2.2';

/***************************************************************
 * ON OPEN – BUILD MENU + OPTIONAL VERSION WRITE
 ***************************************************************/
function onOpen() {
  ...
  // Optional: write version into Settings!B12 (label in A12)
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const settings = ss.getSheetByName('Settings');
    if (settings) {
      settings.getRange('A12').setValue('Script Version');
      settings.getRange('B12').setValue(CANVAS_HUB_VERSION);
    }
  } catch (e) {
    Logger.log('Could not write version to Settings: ' + e.message);
  }
}
 ...
 * Called from the Canvas Hub menu.
 */
...
function buildMissingDialogHtml_(coursePairs) {
    ...
    <html>
      <head>
        ...
        <style>
          ...
          select, button { width: 100%; padding: 8px; font-size: 13px; box-sizing: border-box; }
  ...
  html.setWidth(380).setHeight(330);
  return html;
}
...
/**
 * Simple helper; returns the declared version as a string.
 */
